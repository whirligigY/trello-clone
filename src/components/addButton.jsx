...
const onChange =(ev)=> {
  ...
  const renderForm = (type, text)=> {
      const placeholder = type === "list" ? 'Введите заголовок списка' : 'Введите заголовок для это карточки';
...
const renderButton = (type,text) => {
      return (
        <div style = {type ==='list' ? style.list : style.card} onClick = {()=> onChangeCondition('isFormActive', true)}>
  ...
  return (
    <>
        { condition.isFormActive ? renderForm(type,text, style) : renderButton(type,text)}
    </>
  );
};
const style = {
