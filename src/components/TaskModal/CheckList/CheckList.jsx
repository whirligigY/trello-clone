...
const CheckList = ({
  ...
  useEffect(() => {
    if (listCheckedCheckboxes.length === 0) {
      ...
      setProgress(
        Math.round((listCheckedCheckboxes.length / listCheckboxes.length) * 100)
      );
    }
  ...
  useEffect(() => {
    setListCheckedCheckboxes([
      ...checkedCheckboxes.filter((item) => item.listId == id),
    ]);
  }, [checkedCheckboxes]);
      ...
      {listCheckboxes.length != 0 && (
        <div className="check-list-items">
          {checkboxes.map((item) => {
            if (item.listId == id) {
              return (
                <InputGroup className="mb-3 subtask" key={item.id} id={item.id}>
                  <InputGroup.Checkbox
