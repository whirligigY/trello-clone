...
const LabelsDropdownMenu = ({
  ...
  const setCurrentLabels = () => {
    const tempArr = [];
    labels.map((item) => {
      const index = activeLabels.indexOf(
        activeLabels.find((el) => el.id == item.id)
      );
      if (index != -1) {
        item.status = true;
      } else {
    ...
    setTrueCardLabels(
      tempArr.map((item) => ({ ...item, id: Number(item.id) }))
    );
  };
  useEffect(() => {
    setCurrentLabels();
  ...
  const test = (e) => {
    if (e.target.classList.contains('edit-button')) {
      ...
      e.target.classList.contains('btn-check') ||
  ...
  return (
    <Dropdown.Menu>
      ...
      <div className="labels-list">
        ...
        {/*    <input className={`label ${item.status ? "active" : ""} ${item.color}`}
        disabled value={`${item.value}`} data-color={`${item.color}`}/>*/}
        ...
        {/*})}*/}
        {!search &&
          trueCardLabels.map((item, i) => {
            return (
              <div className="label-item" id={`${item.id}`} key={i}>
                <Button onClick={addLabel}>
                  <input
                    className={`label ${item.status ? 'active' : ''} ${
                      item.color
                    }`}
                    disabled
                    value={`${item.value}`}
                ...
                <Dropdown onClick={test}>
                  <Dropdown.Toggle className="edit-button" />
                  <EditLabel changeLabels={changeLabels} label={item} />
                </Dropdown>
              </div>
            );
          })}
